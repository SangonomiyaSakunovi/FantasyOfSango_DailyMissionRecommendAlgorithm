...
namespace SangoScripts_MaxLikehoodAndEbbinghasuAlgorithm
{
    ...
    /// </summary>
    public class EMDEAlgorithm
    {
        /// <summary>
        /// All the DMR method is under this Instance
        /// </summary>
        public static EMDEAlgorithm Instance;

        private EMDEAlgorithmConfig emdeConfig;
        private ConcurrentDictionary<string, Normal> cacheNormalDistributionDict;

        /// <summary>
        /// The Init method, must call before use other method
        /// </summary>
        /// <param name="emdeAlgorithmConfig">We suggest customize it before init</param>
        public void InitAlgorithm(EMDEAlgorithmConfig emdeAlgorithmConfig = null)
        {
            Instance = this;
            if (emdeAlgorithmConfig == null)
            {
                emdeAlgorithmConfig = new EMDEAlgorithmConfig();
            }
            emdeConfig = emdeAlgorithmConfig;
            InitSettings();
        }
        ...
        public List<EMDEData> GetDMRProbabilityToList(List<EMDEData> userDatas)
        {
            float probRawSum = 0;
            ...
            for (int index = 0; index < userDatas.Count; index++)
            {
                ...
                float dayRetention = CalculateEbbinghausRetention(emdeConfig.PassedDaysEBHS_K_Value, emdeConfig.PassedDaysEBHS_C_Value, data.PassedDays);
                data.EBHS_PassedDay_Probability = data.NormalProbabilityFit * (1 - dayRetention);
                float countRetention = CalculateEbbinghausRetention(emdeConfig.DoneCountEBHS_K_Value, emdeConfig.DoneCountEBHS_C_Value, differDoneCount);
                data.EBHS_DoneCount_Probability = data.NormalProbabilityFit * countRetention;
                data.EBSH_Probability_Raw = (data.EBHS_PassedDay_Probability * emdeConfig.PassedDaysEBHS_Weight + data.EBHS_DoneCount_Probability * emdeConfig.DoneCountEBHS_Weight) /
                    (emdeConfig.PassedDaysEBHS_Weight + emdeConfig.DoneCountEBHS_Weight);
